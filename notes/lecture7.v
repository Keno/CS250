...
Proof.
  ...
  eapply (list_all_imp (le m) (le n)) ; auto.
  intros. 
...
(* First, we need to define a function to merge two (already
   ...
   Fixpoint merge (xs ys:list nat) : list nat := 
      ...
      | [], ys => ys
      | xs, [] => xs
      | h1::t1, h2:t2 -> if nat_lte x y then 
                           h1 :: (merge t1 ys)
                         else
                           h2 :: (merge xs t2)
      end
...
0. Show that 
    ...
    sort_corr xs ys -> sublist ys xs

    Since sort_corr xs ys trivially implies sublist xs ys, 
    this implies that xs and ys have exactly the same elements
    (i.e., that ys is a permutation of xs.)
